                        ...
                        <form method="post" action="{{ route('quizzes.update', $quiz) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-name">{{ __('Quiz Title') }}</label>
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Quiz Title') }}" value="{{ $quiz->name }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('course_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="course_id" id="input-course_id" required class="form-control">
                                        ...
                                        <option <?php if($quiz->course->id == $course->id) echo 'selected'; ?> value="{{$course->id}}">{{\Str::limit($course->title , 40)}}</option>
                                        @endforeach
                                    </select>
